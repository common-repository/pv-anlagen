	...
	<figure>
		<div class="forecast1">
			<div class="slideshow-container">
				<p id="leistungForecast" name="leistung">Prognose: Verbrauch in kWh am <?php echo esc_attr(SEVZ_TODAY) ?></p>
		...
		${'einspeisungki'.$i} = $feedinki[$i];?>
					...
					var dataki = {
						...
						datasets: [{
							...
							label: 'Prognose: Verbrauch in kWh',
							data: einspeisungki,
							backgroundColor: '#8B4577',
							fill: 'origin',
							borderColor: '#535D64',
							borderWidth: 1
						}]
		...
		<script>
			...
			minusBtnForecast.addEventListener('click', function(){
				if (numberDayForecast>min){
					...
					var slides = document.getElementById('leistungForecast').innerHTML = "Prognose: Verbrauch in kWh am " + DateDay;
				}
				if(numberDayForecast == min) {   
					numberDayForecast =-1;
			...
			plusBtnForecast.addEventListener('click', function(){
				if(numberDayForecast ==0){
					...
					dataki = {
						...
						datasets: [{
							label: 'tatsächlicher Verbrauch in kWh',
				...
				if(numberDayForecast == min) {   
					...
					slides = document.getElementById('leistungForecast').innerHTML = "Prognose: Verbrauch in kWh am " + todayDay;
					myLineChartki.destroy();
					dataki = {
						labels: timestampki,
						datasets: [{
