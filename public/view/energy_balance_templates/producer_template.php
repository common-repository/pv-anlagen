<div class="tabbed">
	...
	<figure>
		<script>
			...
			document.getElementById("tab1").onclick = function() {
				var slides = document.getElementById('leistung').innerHTML = "Netzeinspeisung in kWh am " + lastDayDate;
				<?php
				if(!empty($link)){
				?>
				let resultDay;
			...
			document.getElementById("tab2").onclick = function() {
				var slidesMonth = document.getElementById('leistungMonth').innerHTML = "Netzeinspeisung in kWh im Monat " + lastMonthDate;
				myBarChartMonth.data.labels = verbrauch_timestamp_month;
				myBarChartMonth.data.datasets[0].data = einspeisung_delta_month;
				myBarChartMonth.update();
			};
			document.getElementById("tab3").onclick = function() {
				var slidesYear = document.getElementById('leistungYear').innerHTML = "Netzeinspeisung in kWh im Jahr " + lastYearDate;
				myBarChartYear.data.labels = verbrauch_timestamp_year;
				myBarChartYear.data.datasets[0].data = einspeisung_delta_year;
			...
			<div class="slideshow-container">
				<div class="mySlides">		
					...
					<form id="radioform" method="post">
						<p id="leistung" name="leistung">Netzeinspeisung in kWh am <?php echo esc_attr($dateLeistung) ?></p>
					</form>
								...
								({
									...
									success: function (obj){
										...
										if (obj) {
											...
											myLineChart.data.datasets[0].data = einspeisung_delta;
											myLineChart.update();
				...
				<?php
				// Slider charts for every day 1 week/ month?!!!
				for($j=30; $j>=0; $j--){
					$chartId= "myChart".$j;
				...
				?>
				<div class="mySlides">
					<p id="leistung" name="leistung">Netzeinspeisung in kWh am <?php echo $allDates[$j] ?></p>
		...
		<div class="tab2">
			...
			<div class="slideshow-container">
				...
				?>
				<p id="leistungMonth">Netzeinspeisung in kWh im Monat <?php echo esc_attr($dateLeistungMonth) ?></p>
	...
	for($i = 0; $i<= count($timestampMonth)-1; $i++){
		...
		<script>
			...
			plusBtnMonth.addEventListener('click', function(){
				if(numberMonth<maxMonth){
					...
					myBarChartMonth.data.datasets[0].data = einspeisung_delta_monthFor;
					myBarChartMonth.update();

					var slidesMonth = document.getElementById('leistungMonth').innerHTML = "Netzeinspeisung in kWh im Monat " + DateDay;
				}     
				if(numberMonth == maxMonth){
				}
		...
		<div class="tab3">
			...
			<div class="slideshow-container">
				...
				?>
				<p id="leistungYear">Netzeinspeisung in kWh im Jahr <?php echo esc_attr($dateLeistungYear) ?></p>
	...
	for($i = 0; $i<= count($timestampYear)-1; $i++){
		...
		<script>	
			...
			plusBtnYear.addEventListener('click', function(){
				if(numberYear<maxYear){
					...
					myBarChartYear.data.datasets[0].data = einspeisung_delta_yearFor;
					myBarChartYear.update();

					var slidesYear = document.getElementById('leistungYear').innerHTML = "Netzeinspeisung in kWh im Jahr " + lastYear;
				}     
				if(numberYear == maxYear){
				}
		...
		<div class="tab4">
			...
			<p id="leistungGesamt" name="leistungGesamt">Netzeinspeisung in kWh gesamt</p>
			...
			<script>
				...
				function addData() {
					...
					myBarChartSum.data.datasets[0].data.push(einspeisung_delta_sum1);
					myBarChartSum.update();
				}
				<?php
	if($feedinYearPrevSum > 0){
				?>
