<div class="tabbed">
	...
	<figure>
		...
		<script>
			...
			document.getElementById("tab1").onclick = function() {
				var slides = document.getElementById('leistung').innerHTML = "Netzeinspeisung, Verbrauch in kWh am  " + lastDayDate;
				<?php
				if(!empty($link)){
				?>
				let resultDay;
			...
			document.getElementById("tab2").onclick = function() {
				var slidesMonth = document.getElementById('leistungMonth').innerHTML = "Netzeinspeisung, Verbrauch in kWh im Monat " + lastMonthDate;
				myBarChartMonth.data.labels = verbrauch_timestamp_month;
				myBarChartMonth.data.datasets[0].data = verbrauch_delta_month;
				myBarChartMonth.data.datasets[1].data = einspeisung_delta_month;
				myBarChartMonth.update();
			};
			document.getElementById("tab3").onclick = function() {
				var slidesYear = document.getElementById('leistungYear').innerHTML = "Netzeinspeisung, Verbrauch in kWh im Jahr " + lastYearDate;
				myBarChartYear.data.labels = verbrauch_timestamp_year;
				myBarChartYear.data.datasets[0].data = verbrauch_delta_year;
				myBarChartYear.data.datasets[1].data = einspeisung_delta_year;
			...
			<div class="slideshow-container">
				<div class="mySlides">		
					...
					<form id="radioform" method="post">
						<p id="leistung" name="leistung">Netzeinspeisung, Verbrauch in kWh am <?php echo esc_attr($dateLeistung) ?></p>
					</form>	
						...
						var data = {
							...
							datasets: [{
								...
								borderWidth: 1
									   ...
									   {
										   label: 'Netzeinspeisung in kWh',
										   data: [],
										   backgroundColor: '<?php echo esc_attr(get_option('chartfeedin'));?>',
										   fill: 'origin',
										   borderColor: '#535D64',
								...
								({
									...
									success: function (obj){
										...
										if (obj) {
											...
											myLineChart.data.datasets[1].data = einspeisung_delta;
											myLineChart.update();
				...
				<?php
				// Slider charts for every day 1 month
				for($j=30; $j>=0; $j--){
					$chartId= "myChart".$j;

					$last_day_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM `{$wpdb->base_prefix}{$table_name}_day_{$benutzername1}_{$user_id}` WHERE DAY(`zeitstempel`)=%d AND MONTH(`zeitstempel`)=%d AND YEAR(`zeitstempel`) =%d ", $allDatesDay[$j], $allDatesMonth[$j], $allDatesYear[$j]));

				?>
				<div class="mySlides">
					<p id="leistung" name="leistung">Netzeinspeisung, Verbrauch in kWh am <?php echo $allDates[$j] ?></p>
		...
		<div class="tab2">
			...
			<div class="slideshow-container">
				...
				?>
				<p id="leistungMonth">Netzeinspeisung, Verbrauch in kWh im Monat <?php echo esc_attr($dateLeistungMonth) ?></p>
	...
	for($i = 0; $i<= count($timestampMonth)-1; $i++){
		...
		<script>
			...
			plusBtnMonth.addEventListener('click', function(){
				if(numberMonth<maxMonth){
					...
					myBarChartMonth.data.datasets[1].data = einspeisung_delta_monthFor;
					myBarChartMonth.update();

					var slidesMonth = document.getElementById('leistungMonth').innerHTML = "Netzeinspeisung, Verbrauch in kWh im Monat " + DateDay;
				}     
				if(numberMonth == maxMonth){
				}
		...
		<div class="tab3">
			...
			<div class="slideshow-container">
				<?php
				$dateLeistungYear = SEVZ_YEAR;
				?>
				<p id="leistungYear">Netzeinspeisung, Verbrauch in kWh im Jahr <?php echo esc_attr($dateLeistungYear) ?></p>
	...
	for($i = 0; $i<= count($timestampYear)-1; $i++){
		...
		<script>	
			...
			plusBtnYear.addEventListener('click', function(){
				if(numberYear<maxYear){
					...
					myBarChartYear.data.datasets[1].data = einspeisung_delta_yearFor;
					myBarChartYear.update();

					var slidesYear = document.getElementById('leistungYear').innerHTML = "Netzeinspeisung, Verbrauch in kWh im Jahr " + lastYear;
				}     
				if(numberYear == maxYear){
				}
		...
		<div class="tab4">
			...
			<p id="leistungGesamt" name="leistungGesamt">Netzeinspeisung, Verbrauch in kWh gesamt</p>
			...
			<script>
				...
				function addData() {
					...
					myBarChartSum.data.datasets[1].data.push(einspeisung_delta_sum1);
