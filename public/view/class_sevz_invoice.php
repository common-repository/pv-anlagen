...
class SEVZ_Add_invoice {
	...
	function __construct($customer_data) {
		...
		if($startContract){
			...
			$startContract = date('01.01.'.$aYearPrev);
		}
		$thisYear = date('Y', $now);
		$allDates = Array();
		$allMonthDates = Array();
		$prevMonth = strtotime('-1 Month', $now);
		$page = get_page_by_title( 'Rechnung' ); //as an e.g.
		$page_id = $page->ID;
...
<div class="orders">
	...
	<div class="month">
		<details>
			...
			<div class="wp-block-file">
				<a href="<?php echo esc_url(get_home_url().'/?page_id='.$page_id.'&lastmonth_calculate='.$startContract.'&lastyear_calculate='.$allYearDates[$i])?>" target="_blank" rel="noreferrer noopener">Gesamtrechnung Jahr: 
		...
		for ($j = 0; $j<=12; $j++ ){
			...
			?>
			<div class="wp-block-file">
				<a href="<?php echo esc_url(get_home_url().'/?page_id='.$page_id.'&lastmonth_calculate='.$allMonthDates[$j].'&lastyear_calculate='.$allYearDates[$j])?>" target="_blank" rel="noreferrer noopener">Rechnung Monat: 
	...
	<div class="month">
		<details>
			...
			<div class="wp-block-file">
				<a href="<?php echo esc_url(get_home_url().'/?page_id='.$page_id.'&lastmonth_calculate='.$allMonthDates[$i].'&lastyear_calculate='.$allYearDates[$i])?>" target="_blank" rel="noreferrer noopener">Rechnung Monat: 
