...
class SEVZ_Add_producer{

	function my_add_submenu_page_content() {


		global $wpdb;

		$betreiber = $wpdb->get_results("SELECT `ID`,`user_login` FROM `{$wpdb->base_prefix}users` ORDER BY `{$wpdb->base_prefix}users`.`ID` ASC");
		$betreiberID = $wpdb->get_results("SELECT `user_id` FROM `{$wpdb->base_prefix}usermeta` WHERE `meta_value` = 'a:1:{s:8:\"producer\";b:1;}'");
...
<form action= "<?php get_page_by_title($pagetitle)?>"; method="post" enctype="multipart/form-data">
	<div class="fancy-input">
		...
		<select name = "betreiber_name" id = "betreiber_name">
			...
			?>
			<option value="<?php echo esc_html($userName2); $current_id = esc_html($value->ID); ?>">
	...
	<div class="smart-input">
		...
		<div class="smart-input" id = "poweroptiuser"  >
			...
			<input id= "username" type="text" name="username" value="<?php echo esc_textarea($username) ?>"/><br />
			<label for="password">Passwort:<span class="error">* </span></label>
			<input type="password" name="passwort" value="<?php echo esc_html($smartpasswort) ?>" />
		</div>
		<div class="fancy-input" id = "zaehlernr" style="display: none;" >
			...
			<input id = "zaehler" type="text" name="smartname" placeholder="z.B. 1EMH0011822817" value="<?php echo esc_textarea($smartname) ?>"/>
		</div>
	</div><br />
	<div class="fancy-input">
